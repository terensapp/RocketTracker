...
- **The USB cutout is centered on the electronics bay's width by construction.** It used to be a hardcoded guess (`usb_offset = 6.0`) that was noticeably off-center. It's now derived from the board pocket's actual width (`usb_offset = (elec_pocket_w - usb_w) / 2`), so it stays centered even if you change the board clearance elsewhere in the file. If your board's USB port genuinely isn't centered on the board width, nudge `usb_center_bias` (not `usb_offset` directly) to shift it.
- **The inline power switch mount is sized for a specific part** — an **SS12D00-G4** mini slide switch (body 8.7 x 3.7 x 3.7mm, lever 1.5 x 1.5 x 4mm, from the manufacturer's dimensioned drawing) — instead of a generic guessed cutout. It's a keyed pocket on the battery bay's front wall, centered along that bay's length: a narrow outer slot only the slide lever pokes through, and a wider pocket behind it (open to the battery bay's interior) that captures the switch body, so it can't pull back out through the narrow slot. Two small retention bumps (the same idea as the lid's snap bumps, just scaled down) protrude into the pocket from its top and bottom faces — press the switch body in from inside the case and it snaps past them with a "click" and stays put, no glue needed to keep it from rattling loose (though a dab is still fine as extra insurance). It's positioned away from the tie tab on purpose — an earlier version of this case put the switch cutout on the tie tab's end wall, which looked like a stray hole and weakened the same wall the tie tab needs to stay solid. **Height note:** the switch mount sits in the headroom above the battery, not centered in the full case height — an earlier version centered it in the full height, which overlapped the battery's footprint by about 0.85mm and blocked it from sliding fully into the bay. The mount now derives its height from the actual battery envelope (`batt_top_z`) so it can't collide with the battery regardless of other dimensions. If you use a different slide switch, resize `sw_body_l` / `sw_body_w` / `sw_body_h` / `sw_lever_w` at the top of the file to match its drawing (and check `sw_bump_r`/`sw_bump_protrude` still make sense for its body thickness).
